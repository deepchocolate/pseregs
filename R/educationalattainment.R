...
#' them into three levels where compulsory is equivalient of at most 9 years of education, secondary 9 to 12 years, and
...
#' sunCodes <- c('001','200','312', '415','999', NA)
...
EA <- function (object, attainments, codeLevel, missingCode='999') {
  ...
  outputs[!is.na(outputs) & codes == '3'] <- attainments[['secondary']]
  outputs[!is.na(outputs) & codes >= '4'] <- attainments[['tertiary']]
